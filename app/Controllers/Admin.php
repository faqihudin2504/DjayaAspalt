...
class Admin extends BaseController
{
    ...
    // --- CRUD Pelanggan ---
    public function tambahPelanggan() { return view('admin/tambah_pelanggan', ['page_title' => 'Tambah Pelanggan Baru']); }
    ...
    public function editPelanggan($id) { $model = new PelangganModel(); $data = ['page_title' => 'Edit Pelanggan', 'pelanggan' => $model->find($id)]; return view('admin/edit_pelanggan', $data); }
    ...
    public function viewPelanggan($id) { $model = new PelangganModel(); $data = ['page_title' => 'Detail Pelanggan', 'pelanggan' => $model->find($id)]; return view('admin/view_pelanggan', $data); }
    ...
    public function editSurvey($id) { $surveyModel = new SurveyModel(); $pelangganModel = new PelangganModel(); $data = ['page_title' => 'Edit Data Survey', 'survey' => $surveyModel->find($id), 'pelanggan_list' => $pelangganModel->findAll()]; return view('admin/edit_survey', $data); }
    ...
    // --- CRUD Alat & Material ---
    public function tambahAlat() { return view('admin/tambah_alat', ['page_title' => 'Tambah Data Baru']); }
    public function editAlat($id) { $model = new AlatModel(); return view('admin/edit_alat', ['page_title' => 'Edit Data Alat', 'alat' => $model->find($id)]); }
    ...
    // --- CRUD Paket ---
    public function tambahPaket() { return view('admin/tambah_paket', ['page_title' => 'Tambah Paket Baru']); }
    ...
    public function editPaket($id) { $model = new PaketModel(); return view('admin/edit_paket', ['page_title' => 'Edit Paket', 'paket' => $model->find($id)]); }
    ...
    public function hapusPaket($id) { $model = new PaketModel(); try { $model->delete($id); return redirect()->back()->with('success', 'Data berhasil dihapus.'); } catch (\Exception $e) { return redirect()->back()->with('show_error_modal', true); } }
    
    // --- CRUD Pemesanan ---
    public function tambahPemesanan() { $model = new PelangganModel(); return view('admin/tambah_pemesanan', ['page_title' => 'Tambah Pemesanan', 'pelanggan_list' => $model->findAll()]); }
    ...
    public function editPemesanan($id) { $pemesananModel = new PemesananModel(); $pelangganModel = new PelangganModel(); $data = ['page_title' => 'Edit Data Pemesanan', 'pemesanan' => $pemesananModel->find($id), 'pelanggan_list' => $pelangganModel->findAll()]; return view('admin/edit_pemesanan', $data); }
    ...
    public function editPenyewaan($id) { $penyewaanModel = new PenyewaanModel(); $pelangganModel = new PelangganModel(); $alatModel = new AlatModel(); $data = ['page_title' => 'Edit Penyewaan', 'penyewaan' => $penyewaanModel->find($id), 'pelanggan_list' => $pelangganModel->findAll(), 'alat_list' => $alatModel->findAll()]; return view('admin/edit_penyewaan', $data); }
    ...
    public function viewPenyewaan($id) { $model = new PenyewaanModel(); $data = ['page_title' => 'Detail Penyewaan', 'penyewaan' => $model->find($id)]; return view('admin/view_penyewaan', $data); }
    ...
    // --- CRUD Pembayaran ---
    public function tambahPembayaranPemesanan() { $model = new PemesananModel(); $data = ['page_title' => 'Tambah Pembayaran Pemesanan', 'transaksi_list' => $model->getPemesananWithDetails(), 'tipe' => 'pemesanan']; return view('admin/pembayaran_tambah', $data); }
    public function tambahPembayaranPenyewaan() { $model = new PenyewaanModel(); $data = ['page_title' => 'Tambah Pembayaran Penyewaan', 'transaksi_list' => $model->getPenyewaanWithDetails(), 'tipe' => 'penyewaan']; return view('admin/pembayaran_tambah', $data); }
    public function simpanPembayaran() { $model = new PembayaranModel(); $data = $this->request->getPost(); $data['id_bayar'] = 'PAY' . date('ymdHis'); if ($buktiFile = $this->request->getFile('bukti_pembayaran')) { if ($buktiFile->isValid() && !$buktiFile->hasMoved()) { $newName = $buktiFile->getRandomName(); $buktiFile->move(FCPATH . 'uploads/bukti', $newName); $data['bukti_pembayaran'] = $newName; } } $model->save($data); $redirectUrl = ($data['tipe'] === 'penyewaan') ? 'admin/pembayaran/penyewaan' : 'admin/pembayaran/pemesanan'; return redirect()->to($redirectUrl)->with('success', 'Data pembayaran berhasil direkam.'); }
    ...
    // --- CRUD Pengembalian ---
    public function tambahPengembalian() { $model = new PenyewaanModel(); return view('admin/tambah_pengembalian', ['page_title' => 'Tambah Data Pengembalian', 'penyewaan_list' => $model->where('status', 'Disewa')->findAll()]); }
