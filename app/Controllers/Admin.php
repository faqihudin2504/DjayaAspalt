...
class Admin extends BaseController
{
    ...
    public function manajemenAlat()
    {
        ...
        $builder = $alatModel;
        if ($kategori === 'alat-berat') {
            $builder->where('kategori', 'Alat Berat');
        } elseif ($kategori === 'material') {
            $builder->where('kategori', 'Material');
        }

        $data = [
            'page_title' => 'Manajemen Alat & Material',
            'alat_list'  => $builder->orderBy('kategori', 'ASC')->orderBy('nama_alat', 'ASC')->findAll(),
            'kategori_aktif' => $kategori ?? 'semua',
        ];
    ...
    public function tambahAlat() { return view('admin/tambah_alat', ['page_title' => 'Tambah Data Baru', 'back_url' => 'admin/alat']); }
    public function editAlat($id) { $model = new AlatModel(); $alat = $model->find($id); $back_url = 'admin/alat?kategori=' . urlencode(strtolower(str_replace(' ', '-', $alat['kategori']))); return view('admin/edit_alat', ['page_title' => 'Edit Data Alat', 'alat' => $alat, 'back_url' => $back_url]); }
    public function simpanAlat() { $model = new AlatModel(); $data = $this->request->getPost(); if ($gambar = $this->request->getFile('gambar_alat')) { if ($gambar->isValid() && !$gambar->hasMoved()) { $newName = $gambar->getRandomName(); $gambar->move(FCPATH . 'uploads/alat', $newName); $data['gambar_alat'] = $newName; } } $model->save($data); $redirect = ($data['kategori'] === 'Material') ? 'admin/alat?kategori=material' : 'admin/alat?kategori=alat-berat'; return redirect()->to($redirect)->with('success', 'Data berhasil disimpan.'); }
    public function updateAlat($id) { $model = new AlatModel(); $data = $this->request->getPost(); if ($gambar = $this->request->getFile('gambar_alat')) { if ($gambar->isValid() && !$gambar->hasMoved()) { $alatLama = $model->find($id); if ($alatLama && !empty($alatLama['gambar_alat'])) { @unlink(FCPATH . 'uploads/alat/' . $alatLama['gambar_alat']); } $newName = $gambar->getRandomName(); $gambar->move(FCPATH . 'uploads/alat', $newName); $data['gambar_alat'] = $newName; } } $model->update($id, $data); $redirect = ($data['kategori'] === 'Material') ? 'admin/alat?kategori=material' : 'admin/alat?kategori=alat-berat'; return redirect()->to($redirect)->with('success', 'Data berhasil diperbarui.'); }
    ...
    public function tambahPemesanan() { $pelangganModel = new PelangganModel(); $alatModel = new AlatModel(); $data = [ 'page_title' => 'Tambah Pemesanan', 'pelanggan_list' => $pelangganModel->findAll(),'material_list' => $alatModel->where('kategori', 'Material')->findAll() ]; return view('admin/tambah_pemesanan', $data); }
    public function viewPemesanan($id) { $pemesananModel = new \App\Models\PemesananModel(); $detailModel = new \App\Models\DetailPemesananModel(); $pemesanan = $pemesananModel->select('pemesanan.*, pelanggan.nama_lengkap, pelanggan.no_telpon') ->join('pelanggan', 'pelanggan.id_pelanggan = pemesanan.id_pelanggan', 'left') ->where('pemesanan.id_pesanan', $id) ->first(); if (!$pemesanan) { throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound(); } $detail_material = $detailModel->where('id_pesanan', $id)->findAll(); $totalHargaMaterial = 0; foreach ($detail_material as $item) { $totalHargaMaterial += $item['sub_total']; } $pemesanan['total_harga_akhir'] = $pemesanan['harga_paketdipesan'] + $totalHargaMaterial; $data = [ 'page_title' => 'Detail Pemesanan', 'pemesanan' => $pemesanan, 'detail_material' => $detail_material, 'back_url' => 'admin/pemesanan']; return view('admin/view_pemesanan', $data); }
    public function simpanPemesanan() { $db = \Config\Database::connect(); $pemesananModel = new \App\Models\PemesananModel(); $detailPemesananModel = new \App\Models\DetailPemesananModel(); $db->transStart(); try { $idPesananBaru = 'PES' . date('ymdHis'); $hargaPaket = (int)$this->request->getPost('harga_paketdipesan'); $totalHargaMaterial = 0; $materials = $this->request->getPost('material'); if (!empty($materials)) { foreach ($materials as $material) { if (empty($material['id']) || empty($material['jumlah'])) continue; $subTotal = (int)$material['jumlah'] * (int)$material['harga']; $detailPemesananModel->save([ 'id_pesanan' => $idPesananBaru, 'id_material' => $material['id'], 'jumlah_material' => $material['jumlah'], 'harga_saat_pesan' => $material['harga'], 'sub_total' => $subTotal ]); $totalHargaMaterial += $subTotal; } } $pemesananData = [ 'id_pesanan' => $idPesananBaru, 'id_pelanggan' => $this->request->getPost('id_pelanggan'), 'nama_paketdipesan' => $this->request->getPost('nama_paketdipesan'), 'harga_paketdipesan'=> $hargaPaket, 'tanggal_pemesanan' => $this->request->getPost('tanggal_pemesanan'), 'total_harga_akhir' => $hargaPaket + $totalHargaMaterial ]; $pemesananModel->save($pemesananData); $db->transComplete(); session()->setFlashdata('success', 'Data pemesanan baru berhasil ditambahkan.'); return redirect()->to('/admin/pemesanan'); } catch (\Exception $e) { $db->transRollback(); log_message('error', '[SIMPAN PEMESANAN] ' . $e->getMessage()); return redirect()->back()->withInput()->with('error', 'Terjadi kesalahan saat menyimpan data.'); } }
    ...
    public function tambahPembayaranPemesanan() { $pemesananModel = new PemesananModel(); $transaksi_list = $pemesananModel->select('pemesanan.*, pelanggan.nama_lengkap') ->join('pelanggan', 'pelanggan.id_pelanggan = pemesanan.id_pelanggan', 'left') ->orderBy('pemesanan.tanggal_pemesanan', 'DESC') ->findAll(); $data = [ 'page_title' => 'Tambah Pembayaran Pemesanan', 'transaksi_list' => $transaksi_list, 'tipe' => 'pemesanan', 'back_url' => 'admin/pembayaran/pemesanan' ]; return view('admin/pembayaran_tambah', $data); }
    ...
    public function simpanPembayaran() { $model = new \App\Models\PembayaranModel(); $data = $this->request->getPost(); $data['id_bayar'] = 'PAY' . date('ymdHis'); $buktiFile = $this->request->getFile('bukti_pembayaran'); if ($buktiFile && $buktiFile->isValid() && !$buktiFile->hasMoved()) { $newName = $buktiFile->getRandomName(); $buktiFile->move(FCPATH . 'uploads/bukti', $newName); $data['bukti_pembayaran'] = $newName; } $model->save($data); session()->setFlashdata('success', 'Data pembayaran berhasil direkam.'); $redirectUrl = ($this->request->getPost('tipe') === 'penyewaan') ? 'admin/pembayaran/penyewaan' : 'admin/pembayaran/pemesanan'; return redirect()->to($redirectUrl); }
    ...
    public function konfirmasiPembayaran($id_bayar = null) { if ($id_bayar === null) { return redirect()->back()->with('error', 'ID Pembayaran tidak valid.'); } $pembayaranModel = new PembayaranModel(); $pembayaranModel->update($id_bayar, ['status_pembayaran' => 'Lunas']); return redirect()->back()->with('success', 'Pembayaran berhasil dikonfirmasi menjadi Lunas.'); }
